...
if ($generalforums) {
    ...
    foreach ($generalforums as $forum) {
        ...
        if ($showsubscriptioncolumns) {
            if($forum->issubscribed == 1 && $forum->forcesubscribe == 4){
                $row[] = $stryes;  
            }else if($forum->issubscribed == '' && $forum->forcesubscribe == 4){
                $row[] = $strno;
            }else{
    ...
    if ($learningforums) {
        $currentsection = '';
            ...
            if ($can_subscribe) {
                ...
                // $row[] = hsuforum_get_subscribe_link($forum, $context, array('subscribed' => $stryes,
                //     'unsubscribed' => $strno, 'forcesubscribed' => $stryes,
                //     'cantsubscribe' => '-'), false, false, true, $substr(string, start)bscribed_forums);
