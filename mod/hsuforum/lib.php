...
function hsuforum_recent_activity($course, $viewfullnames, $timestart, $forumid = null) {
    ...
    foreach ($posts as $post) {
        ...
        if (hsuforum_is_user_group_discussion($cm, $post->groupid)) {
            ...
            if($DB->record_exists('hsuforum_subscriptions',array('userid'=>$USER->id,'forum'=>$forumid)) || is_siteadmin()) {
                $out .= hsuforum_media_object($url, $picture, $postusername, $posttime, $postsubject);
            }else{
                $out .= hsuforum_media_object_static($url, $picture, $postusername, $posttime, $postsubject);
            }
        }
...
function hsuforum_media_object_static($url, $picture, $username, $time, $subject) {
        $out = "<div class=\"snap-media-object\">";
        $out .= $picture;
        $out .= "<div class=\"snap-media-body\">";
        $out .= "<h3>".format_string($username)."</h3>";
 ...
 * @return string
 */
function hsuforum_get_subscribe_link($forum, $context, $messages = array(), $cantaccessagroup = true, $fakelink=true, $backtoindex=false, $subscribed_forums=null) {
...
// and the current user is a guest.
    ...
    if ($canstart) {
        if(($DB->record_exists('hsuforum_subscriptions',array('userid'=>$USER->id,'forum'=>$forum->id))) || is_siteadmin()) {
...
function hsuforum_extend_settings_navigation(settings_navigation $settingsnav, navigation_node $forumnode) {
    ...
    //By Arjun
    if ($cansubscribe) {
        if($forumobject->forcesubscribe == 4){
