...
class mod_hsuforum_renderer extends plugin_renderer_base {
    ...
    public function render_discussionsview($forum) {
        ...
        $f = $DB->get_record('hsuforum',array('id'=>$forum->id));
        if(has_capability('mod/hsuforum:viewsubscribers', $context)){
            $url = new \moodle_url('/mod/hsuforum/subscribers.php', ['id' => $forum->id]);
            $manageforumsubscriptions = get_string('showsubscribers', 'mod_hsuforum');
            $output .= '<div class="text-right"><hr>';
            $output .= \html_writer::link($url, $manageforumsubscriptions, array('class' => 'btn btn-link'));
            $output .= '</div>'; 
        }else{
            if($f->forcesubscribe == 4){

                if($DB->record_exists('hsuforum_subscriptions',array('userid'=>$USER->id,'forum'=>$forum->id))) {
                    ...
                    if($req = $DB->record_exists('hsuforum_requests',array('userid'=>$USER->id,'forumid'=>$forum->id))) {
                        $output .= '<button type="button" class="btn btn-primary" disabled>Subscription request sent</button>';
                    }else{
                        $dsort = hsuforum_lib_discussion_sort::get_from_session($forum, $context);
                        $output .= '<form method="get" action="'.$CFG->wwwroot.'/mod/hsuforum/request.php">
                                        ...
                                        <button type="submit" class="btn btn-primary" title="">Subscribe to this forum</button>
                                    </form>';
                    }
                }  
            }else{
    ...
    public function discussion_template($d, $forumtype) {
        ...
        if (!$d->fullthread) {
            if($statusvalue == 1){
                ...
                $forum = $DB->get_record('hsuforum_discussions',array('id'=>$d->id),'forum');
                if(($DB->record_exists('hsuforum_subscriptions',array('userid'=>$USER->id,'forum'=>$forum->forum)))|| is_siteadmin()) {
                    $threadtitle = "<a class='disable-router'  href='$d->viewurl'>$threadtitle</a>";
                 }else{
                   $threadtitle = $d->subject; 
                }
