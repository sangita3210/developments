 ...
 */
     ...
     * @return void
     */
    function local_proxyenrolment_extend_settings_navigation(settings_navigation $navigation, $context)
    {
        ...
        // Add our links
        $useradmin_node->add(
            get_string('IMPORT_MENU_LONG', local_proxyenrolment_plugin::PLUGIN_NAME),
            local_proxyenrolment_plugin::get_plugin_url('import', $context->instanceid),
            navigation_node::TYPE_SETTING,
            get_string('IMPORT_MENU_SHORT', local_proxyenrolment_plugin::PLUGIN_NAME),
            null, new pix_icon('i/import', 'import'));

        $useradmin_node->add(
            get_string('ASSIGN_MENU_LONG', local_proxyenrolment_plugin::PLUGIN_NAME),
            local_proxyenrolment_plugin::get_plugin_url('assign', $context->instanceid),
            navigation_node::TYPE_SETTING,
            get_string('ASSIGN_MENU_SHORT', local_proxyenrolment_plugin::PLUGIN_NAME),
            null, new pix_icon('t/move', 'assign'));
    }
    class local_proxyenrolment_plugin
    {
       /*
         * Class constants
         */

        const PLUGIN_NAME                 = 'local_proxyenrolment';
        const PLUGIN_PATH                 = 'local/proxyenrolment';
        ...
        public static function get_group_prefs($course_id)
        {
            ...
            $record_object = $DB->get_record('local_proxyenrol_metagroup', array('course' => $course_id), 'course, data');
        ...
        public static function set_group_prefs($course_id, $prefs = array())
        {
            global $DB;
            try {
                $DB->delete_records('local_proxyenrol_metagroup', array('course' => $course_id));

                $record_object = new stdClass();
                $record_object->course = $course_id;
                $record_object->data   = serialize((array)$prefs);
                $DB->insert_record('local_proxyenrol_metagroup', $record_object, false);
            }
            catch (Exception $exc) {
