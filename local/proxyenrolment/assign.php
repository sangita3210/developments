 ...
 */
    ...
    $page_head_title = get_string('LBL_ASSIGN_TITLE', local_proxyenrolment_plugin::PLUGIN_NAME) . ' : ' . $COURSE->shortname;

    $PAGE->set_title($page_head_title);
    $PAGE->set_heading($page_head_title);
    $PAGE->set_pagelayout('incourse');
    $PAGE->set_url(local_proxyenrolment_plugin::get_plugin_url('assign', $COURSE->id));
    ...
    // Deal breaker
    if (!$meta_enrols) {
        print_error('ERR_NO_META_ENROL', local_proxyenrolment_plugin::PLUGIN_NAME, $course_url);
    }
    ...
    $data->group_prefs      = local_proxyenrolment_plugin::get_group_prefs($COURSE->id);

    $formdata = null;
    $mform    = new local_proxyenrolment_assign_form(local_proxyenrolment_plugin::get_plugin_url('assign', $COURSE->id)->out(), array('data' => $data));
    ...
    if ($mform->is_cancelled()) {
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading_with_help(get_string('LBL_ASSIGN_TITLE', local_proxyenrolment_plugin::PLUGIN_NAME), 'HELP_PAGE_ASSIGN', local_proxyenrolment_plugin::PLUGIN_NAME);
        ...
        // Collect the input
        $metagroup         = $formdata->{local_proxyenrolment_plugin::FORMID_METAGROUP};
        $remove_current    = intval($formdata->{local_proxyenrolment_plugin::FORMID_REMOVE_CURRENT});

        local_proxyenrolment_plugin::metagroup_assign($COURSE, $metagroup, $remove_current);

        echo $OUTPUT->header();
        echo $OUTPUT->heading_with_help(get_string('LBL_ASSIGN_TITLE', local_proxyenrolment_plugin::PLUGIN_NAME), 'HELP_PAGE_ASSIGN', local_proxyenrolment_plugin::PLUGIN_NAME);

        // Output the processing result
        echo $OUTPUT->notification(get_string('INF_IMPORT_SUCCESS', local_proxyenrolment_plugin::PLUGIN_NAME), 'notifysuccess');
