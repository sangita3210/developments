 ...
 */
    ...
    $course_id = required_param(local_proxyenrolment_plugin::PARAM_COURSE_ID, PARAM_INT);
    ...
    require_capability(local_proxyenrolment_plugin::REQUIRED_CAP, $PAGE->context);
    ...
    $page_head_title = get_string('LBL_IMPORT_TITLE', local_proxyenrolment_plugin::PLUGIN_NAME) . ' : ' . $COURSE->shortname;

    $PAGE->set_title($page_head_title);
    $PAGE->set_heading($page_head_title);
    $PAGE->set_pagelayout('incourse');
    $PAGE->set_url(local_proxyenrolment_plugin::get_plugin_url('import', $COURSE->id));
    ...
    $data->user_id_field_options
                            = local_proxyenrolment_plugin::get_user_id_field_options();
   ...
   // print_object($enrols_enabled);
    ...
    if (null == $manual_enrol_instance) {
        print_error('ERR_NO_MANUAL_ENROL', local_proxyenrolment_plugin::PLUGIN_NAME, $course_url);
    }
    ...
    $file_picker_options = array(
        ...
        'maxbytes'       => local_proxyenrolment_plugin::MAXFILESIZE);

    $formdata = null;
    $mform    = new local_proxyenrolment_index_form(local_proxyenrolment_plugin::get_plugin_url('import', $COURSE->id)->out(), array('data' => $data, 'options' => $file_picker_options));
    ...
    if ($mform->is_cancelled()) {
        ...
        get_file_storage()->delete_area_files($user_context->id, 'user', 'draft', file_get_submitted_draft_itemid(local_proxyenrolment_plugin::FORMID_FILES));
        redirect($course_url);
        ...
        echo $OUTPUT->header();
        echo $OUTPUT->heading_with_help(get_string('LBL_IMPORT_TITLE', local_proxyenrolment_plugin::PLUGIN_NAME), 'HELP_PAGE_IMPORT', local_proxyenrolment_plugin::PLUGIN_NAME);
        ...
        // Collect the input
        $user_id_field     = $formdata->{local_proxyenrolment_plugin::FORMID_USER_ID_FIELD};
        $role_id           = $data->metacourse ? 0
                           : intval($formdata->{local_proxyenrolment_plugin::FORMID_ROLE_ID});
        $group_assign      = intval($formdata->{local_proxyenrolment_plugin::FORMID_GROUP});
        $group_id          = intval($formdata->{local_proxyenrolment_plugin::FORMID_GROUP_ID});
        $group_create      = intval($formdata->{local_proxyenrolment_plugin::FORMID_GROUP_CREATE});

        // Leave the file in the user's draft area since we
        // will not plan to keep it after processing
        $area_files = get_file_storage()->get_area_files($user_context->id, 'user', 'draft', $formdata->{local_proxyenrolment_plugin::FORMID_FILES}, null, false);
        $result = local_proxyenrolment_plugin::import_file($COURSE, $manual_enrol_instance, $user_id_field, $role_id, (boolean)$group_assign, $group_id, (boolean)$group_create, array_shift($area_files));

        // Clean up the file area
        get_file_storage()->delete_area_files($user_context->id, 'user', 'draft', $formdata->{local_proxyenrolment_plugin::FORMID_FILES});
