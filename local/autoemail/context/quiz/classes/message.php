...
class message {
    ...
    public function __construct($qid){
        ...
        $this->quizfields  = $this->get_quiz_fields();
        $this->quizvalues  = $this->get_quiz_values($qid);
    }
    ...
    public function get_quiz_values($qid) {
        global $SITE,$DB,$CFG;

        $quizvalue = $DB->get_record('quiz',array('id' =>$qid));
      //  print_object($coursevalue);
        $values = array();
       // $courselink='<a href='.$CFG->wwwroot.'/course/view.php?id='.$cid.'>Course Link</a>';
        $quizname = $quizvalue->name;
        
        foreach ($this->quizfields as $field) {
            $values[$field] = $$field;
        }
        return $values;
    }
    ...
    public function get_welcome_values() {
        ...
        $resetpasswordlink = \html_writer::link(
            new \moodle_url('/login/forgot_password.php'), get_string('resetpass', 'autoemailcontext_quiz'));
    ...
    public function replace_values($user, $message) {
        ...
        foreach ($this->quizfields as $field) {
            $message = str_replace('[['.$field.']]', $this->quizvalues[$field], $message);
        }
        foreach ($this->welcomefields as $field) {
