...
class local_invoicemail_upload_image_form extends moodleform {
        ...
        if ($files) {
            foreach ($files as $file) {
                if (!in_array($file->get_mimetype(), $supportedtypes)) {
                    $errors['invoiceimage'] = get_string('unsupportedfiletype', 'local_invoicemail');
                }
                if($file->get_filesize() > 1048576){
                        $errors['invoiceimage'] = get_string('filesizelimit', 'local_invoicemail');
                }
            }
        } else {
            $errors['invoiceimage'] = get_string('nofileselected', 'local_invoicemail');
        }
