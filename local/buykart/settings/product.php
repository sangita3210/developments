...
require_once $CFG->dirroot . '/local/buykart/forms/edit_product.php';

$courseid = required_param('id', PARAM_INT);

$PAGE->set_pagelayout('admin');
$PAGE->requires->jquery();
$PAGE->requires->js(new moodle_url($CFG->wwwroot . '/local/buykart/js/settings_product.js'));

// Validate course id
if(!!$courseid) {
	$PAGE->set_url('/local/buykart/settings/product.php', array('id' => $courseid));
...
$mform = new buykart_edit_product_form();
...
if ($mform->is_cancelled()) {
	...
	// course_id and id (the product id) are both unique identifiers in the local_buykart_product table
	$recordExists = $DB->get_record('local_buykart_product', array('course_id' => $data->id), '*', IGNORE_MISSING);
	...
	if(!!$recordExists) {
		...
		// Update local_buykart_product fields	
		$result = $DB->update_record('local_buykart_product', $recordProduct);

		// Update product variations
		$existingVariations = $DB->get_records('local_buykart_variation', array('product_id' => $recordProduct->id));
		...
		foreach ($existingVariations as $v) {
			...
			$DB->update_record('local_buykart_variation', $tempVariation);
		}
		// ----
		...
		for ($i=0; $i < count($recordVariations); $i++) { 
			...
			if(0 < count($existingVariations)) {
				...
				// Update the DB
				$variationResult = $DB->update_record('local_buykart_variation', $recordVariations[$i]);
			} else {
				// No more variations exist in the DB, so we need to insert them!
				$variationResult = $DB->insert_record('local_buykart_variation', $recordVariations[$i]);
			}
		}

	} else {
		// No record exists YET
		// Create a new local_buykart_product row
		$result = $DB->insert_record('local_buykart_product', $recordProduct, true);
		
		if(!!$result) {
			// As well as appropriate product_variation rows
			foreach ($recordVariations as $variation) {
				$variation->product_id = $result;
				$variationResult = $DB->insert_record('local_buykart_variation', $variation);
			}
		}
	}
	...
	$existingProductData = $DB->get_record('local_buykart_product', array('course_id' => $courseid), '*', IGNORE_MISSING);
	...
	if(!!$existingProductData) {
		...
		$existingVariationData = $DB->get_records('local_buykart_variation', array('product_id' => $existingProductData->id));
