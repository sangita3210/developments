<?php
/**
 * buykart Single Transaction Page
 *
 * @package     local
...
$PAGE->set_url($CFG->wwwroot . '/local/buykart/pages/transaction/view.php', $params);

$PAGE->navbar->add(get_string('transactions_title', 'local_moodec'), new moodle_url($CFG->wwwroot . '/local/buykart/pages/transaction/index.php'));
...
if ( (int) $USER->id !== $transaction->get_user_id() ) {
	require_capability('local/buykart:viewalltransactions', $context);
}
...
if( !!$transaction->get_error() && has_capability('local/buykart:viewalltransactions', $context)) {
	printf(
