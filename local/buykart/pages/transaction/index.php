...
require_once $CFG->dirroot . '/local/buykart/classes/transaction_table.php';
...
$PAGE->set_url($CFG->wwwroot . '/local/buykart/pages/transaction/index.php', $params);

// Force the user to login/create an account to access this page
require_login();

if ( !has_capability('local/buykart:viewalltransactions', $context) ) {
	$userID = $USER->id;
}
...
$table = new moodec_transaction_table('buykart-transactions-list');
$table->is_downloading($download, 'transaction-report', 'buykart Transaction Report');
...
if (!$table->is_downloading()) {
    ...
    $url = new moodle_url($CFG->wwwroot . '/local/buykart/pages/transaction/index.php');
    echo $renderer->transaction_filter($params, $url);
}

// Configure the table
$table->define_baseurl(new moodle_url($CFG->wwwroot . '/local/buykart/pages/transaction/index.php', $params));
