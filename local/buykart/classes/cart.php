...
class BuykartCart {
	 ...
	 * @var string
	 */
	const STORAGE_ID = 'BuykartCart';

	/**
	 * The string time version of the Buykart Cart.
	...
	function __construct(){
		...
		if( !!$this->get_transaction_id() ) {
			...
			try {
				$transaction = new BuykartTransaction($this->get_transaction_id());

				if( $transaction->get_status() === BuykartTransaction::STATUS_COMPLETE ) {
					$this->clear();
				}
			} catch (Exception $e) {
	...
	public function refresh(){
		...
		foreach( $this->_products as $pID => $vID ){
			$newProduct = local_buykart_get_product($pID);
	...
	public function add($p, $v = 0){
		...
		$productToAdd = local_buykart_get_product($p);
	...
	public function remove($id) {
		...
		if( $this->check($id) ){
			...
			$productToRemove = local_buykart_get_product($id);
