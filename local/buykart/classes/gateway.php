...
//require_once $CFG->wwwroot.'/../../manager.php';


abstract class BuykartGateway {

	/**
	 * The BuykartTransaction to be handled by this gateway 
	 * @var BuykartTransaction
	 */
	 ...
	 * @param BuykartTransaction|int 	$transaction 	A transaction class or transaction ID
	 */
	function __construct($transaction) {
		...
		if( $transaction instanceof BuykartTransaction ) {
			// We have been passed an existing instance, so use it
			$this->_transaction = $transaction;
		} else {
			// We have been passed the ID, so make a new instance of transaction
			$this->_transaction = new BuykartTransaction($transaction);
		}

		// Get the enrolment plugin
		$this->_enrolPlugin = enrol_get_plugin('buykart');

		// If the buykart enrolment plugin is not found (although that shouldn't happen, as it is a dependency of this plugin) then fall back to manual
	...
	protected function verify_transaction(){
		...
		$this->send_error_to_admin("This is the return from verfy function");
	...
	protected function complete_enrolment(){
		...
		foreach ($this->_transaction->get_items() as $item) {
			...
			if( !$instance ) {
				...
				$this->send_error_to_admin("Buykart enrolment method not active on course ". $product->get_course_id() . ". Transaction #" . $this->_transaction->get_id() . " defaulted to manual enrolment method");
			...
			//inform user that enrolment have been successful
			//$this->send_info_to_user("Enrolment successfull",$this->_transaction->get_user_id());
	...
	protected function send_error_to_admin($subject, $data = array()) {
		...
		$subject = $this->_gatewayName . " ERROR: " . $subject;
		...
		//new html code here
		$messagetext = format_text_email($message, FORMAT_HTML);
		
		email_to_user($admin,$supportuser,$subject,$messagetext,$message);
	}
	...
	protected function send_info_to_user($subject, $userid) {
		//print_object($subject);
		global $CFG,$USER;
		require_once $CFG->libdir . '/eventslib.php';
		...
		$subject = $this->_gatewayName . " INFO: " . $subject;

		$message = get_string('usermsg','local_buykart');
		//new html code here
		$messagetext = format_text_email($message, FORMAT_HTML);
		
		email_to_user($user,$supportuser,$subject,$messagetext,$message);
	}
	
	//new functionality added prashant here 
