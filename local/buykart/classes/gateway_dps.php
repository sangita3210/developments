...
class BuykartGatewayDPS extends BuykartGateway {
	...
	function __construct($transaction) {
		...
		$this->_gatewayName = get_string('payment_dps_title', 'local_buykart');

		$this->_internalGatewayURL = new moodle_url($CFG->wwwroot . '/local/buykart/payment/dps/index.php');

		// Checks if sandbox mode is enabled
		if( !!get_config('local_buykart', 'payment_dps_sandbox') ) {
	...
	public function begin(){
        ...
        $xmlrequest = sprintf(
            ...
            clean_param(get_config('local_buykart', 'payment_dps_userid'), PARAM_CLEAN), // PxPay User ID
            clean_param(get_config('local_buykart', 'payment_dps_key'), PARAM_CLEAN),	// PxPay Key
            clean_param($this->_transaction->get_cost(), PARAM_CLEAN), // Amount
            clean_param(get_config('local_buykart', 'currency'), PARAM_CLEAN),	// Currency
            ...
            new moodle_url($CFG->wwwroot . '/local/buykart/payment/dps/success.php'), // URL Success
            new moodle_url($CFG->wwwroot . '/local/buykart/payment/dps/fail.php') 	// URL Fail
        );
        ...
        $this->_transaction->set_gateway(BUYKART_GATEWAY_DPS);
	...
	public function handle($data = null){
		...
		if ($response->CurrencySettlement != get_config('local_buykart', 'currency')) {
		...
		$html = sprintf('<form action="%s" method="POST" class="payment-gateway gateway--dps">', $this->_internalGatewayURL);
			...
			$html .= sprintf(
		 	...
		 	$html .= sprintf(
		 		...
		 		get_string('button_dps_label', 'local_buykart')
		 	);
