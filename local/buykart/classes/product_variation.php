...
class BuykartProductVariation {
	...
	public function get_duration($format = true){
		if($format) {
			...
			if ($duration < 1) {
				return get_string('enrolment_duration_unlimited', 'local_buykart');
			}

			if (364 < $duration) {
				$years = floor($duration / 365);
				$duration = $duration % 365;
				$output .= $years == 1 ? sprintf(' %d %s ', $years, get_string('enrolment_duration_year', 'local_buykart')) : sprintf(' %d %s ', $years, get_string('enrolment_duration_year_plural', 'local_buykart'));
			}

			if (29 < $duration) {
				$months = floor($duration / 30);
				$duration = $duration % 30;
				$output .= $months == 1 ? sprintf(' %d %s ', $months, get_string('enrolment_duration_month', 'local_buykart')) : sprintf(' %d %s ', $months, get_string('enrolment_duration_month_plural', 'local_buykart'));
			}

			if (6 < $duration) {
				$weeks = floor($duration / 7);
				$duration = $duration % 7;
				$output .= $weeks == 1 ? sprintf(' %d %s ', $weeks, get_string('enrolment_duration_week', 'local_buykart')) : sprintf(' %d %s ', $weeks, get_string('enrolment_duration_week_plural', 'local_buykart'));
			}

			if (0 < $duration) {
				$output .= $duration == 1 ? sprintf(' %d %s ', $duration, get_string('enrolment_duration_day', 'local_buykart')) : sprintf(' %d %s ', $duration, get_string('enrolment_duration_day_plural', 'local_buykart'));
			}
